...
<div class="content d-flex flex-column flex-column-fluid">
   <div class="d-flex flex-column-fluid">
      <div class="container-fluid my-4">
         <div class="d-md-flex align-items-center justify-content-between">
            ...
            <div class="text-end my-2 mt-md-0"><a class="btn btn-outline-primary" href="{{ route('transfer.index') }}">Back</a></div>
         </div>
 ...
 <div class="card">
    ...
    <form action="{{ route('transfer.update',$transferData->id)}}" method="post" enctype="multipart/form-data" id="saleReturnUpdateForm" >
       @csrf
...
<div class="row">
 <div class="col-xl-12">
    <div class="card">
       <div class="row">
          <div class="col-md-4 mb-3">
             ...
             <input type="date" name="date" value="<?php echo date('Y-m-d'); ?>" class="form-control" value="{{ $transferData->date }}">
             @error('date')
        ...
        <input type="hidden" name="from_warehouse_id" value="{{ $transferData->from_warehouse_id }}">
        <input type="hidden" name="to_warehouse_id" value="{{ $transferData->to_warehouse_id }}">
    ...
    <option value="{{ $item->id }}" {{ $transferData->from_warehouse_id == $item->id ? 'selected' : '' }} >{{ $item->name }}</option>
    @endforeach
                </select>
    ...
    @foreach ($warehouses as $item)
    <option value="{{ $item->id }}" {{ $transferData->to_warehouse_id == $item->id ? 'selected' : '' }} >{{ $item->name }}</option>
    @endforeach
                </select>
                <small id="warehouse_error" class="text-danger d-none">Please select the first warehouse.</small>
                </div>
  ...
  <div class="row">
    ...
    @foreach ($transferData->TransferItems as $item)
    <tr data-id={{ $item->id  }}>
    ...
    <div class="card">
             ...
             <table class="table border">
                <tbody>
                   <tr>
                      ...
                      <td class="py-3" id="displayDiscount">TK {{ $transferData->discount }}</td>
                   </tr>
                   <tr>
                      <td class="py-3">Shipping</td>
                      <td class="py-3" id="shippingDisplay">TK {{ $transferData->shipping }}</td>
                   </tr>
                   <tr>
                      <td class="py-3 text-primary">Grand Total</td>
                      <td class="py-3 text-primary" id="grandTotal">TK {{ $transferData->grand_total }}</td>
                      <input type="hidden" id="grandTotalInput" name="grand_total" value="{{ $transferData->grand_total }}">
      ...
      <div class="row">
         <div class="col-md-4">
            ...
            <input type="number" id="inputDiscount" name="discount" class="form-control" value="{{ $transferData->discount }}">
         </div>
         <div class="col-md-4">
            <label class="form-label">Shipping: </label>
            <input type="number" id="inputShipping" name="shipping" class="form-control" value="{{ $transferData->shipping }}">
         </div>
         <div class="col-md-4">
            <div class="form-group w-100">
               ...
               <select name="status" id="status" class="form-control form-select">
                  ...
                  <option value="Transfer" {{ $transferData->status == 'Transfer' ? 'selected' : '' }} >Transfer</option>
                  <option value="Pending"  {{ $transferData->status == 'Pending' ? 'selected' : '' }} >Pending</option>
                  <option value="Ordered" {{ $transferData->status == 'Ordered' ? 'selected' : '' }} >Ordered</option>
               </select>
               @error('status')
                  <span class="text-danger">{{ $message }}</span>
               @enderror
      ...
      <div class="col-md-12 mt-2">
         ...
         <textarea class="form-control" name="note" rows="3" placeholder="Enter Notes">{{ $transferData->note }}</textarea>
      </div>
   </div>
</div>
</div>
