   ...
   <div class="d-flex flex-column-fluid">
      <div class="container-fluid my-4">
         <div class="d-md-flex align-items-center justify-content-between">
            ...
            <div class="text-end my-2 mt-md-0"><a class="btn btn-outline-primary" href="{{ route('purchase.index') }}">Back</a></div>
         </div>


 <div class="card">
    <div class="card-body">
    <form action="{{ route('return-purchase.update',$purchasedata->id)}}" method="post" enctype="multipart/form-data">
       @csrf
...
<div class="row">
 <div class="col-xl-12">
    <div class="card">
       <div class="row">
          <div class="col-md-4 mb-3">
             ...
             <input type="date" name="date" value="<?php echo date('Y-m-d'); ?>" class="form-control" value="{{ $purchasedata->date }}">
             @error('date')
    ...
    <option value="{{ $item->id }}" {{ $purchasedata->warehouse_id == $item->id ? 'selected' : '' }} >{{ $item->name }}</option>
    @endforeach
                </select>
          ...
          <div class="col-md-4 mb-3">
             <div class="form-group w-100">
                ...
                <select name="supplier_id" id="supplier_id" class="form-control form-select" >
                   ...
                   <option value="{{ $item->id }}" {{ $purchasedata->supplier_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                   @endforeach
                </select>
             </div>
          </div>
    ...
    <div class="card">
             ...
             <table class="table border">
                <tbody>
                   <tr>
                      ...
                      <td class="py-3" id="displayDiscount">TK {{ $purchasedata->discount }}</td>
                   </tr>
                   <tr>
                      <td class="py-3">Shipping</td>
                      <td class="py-3" id="shippingDisplay">TK {{ $purchasedata->shipping }}</td>
                   </tr>
                   <tr>
                      <td class="py-3 text-primary">Grand Total</td>
                      <td class="py-3 text-primary" id="grandTotal">TK {{ $purchasedata->grand_total }}</td>
                      <input type="hidden" id="grandTotalInput" name="grand_total" value="{{ $purchasedata->grand_total }}">
      ...
      <div class="row">
         <div class="col-md-4">
            ...
            <input type="number" id="inputDiscount" name="discount" class="form-control" value="{{ $purchasedata->discount }}">
         </div>
         <div class="col-md-4">
            <label class="form-label">Shipping: </label>
            <input type="number" id="inputShipping" name="shipping" class="form-control" value="{{ $purchasedata->shipping }}">
         </div>
         <div class="col-md-4">
            <div class="form-group w-100">
               ...
               <select name="status" id="status" class="form-control form-select">
                  ...
                  <option value="Return" {{ $purchasedata->status == 'Return' ? 'selected' : '' }} >Return</option>
                  <option value="Pending"  {{ $purchasedata->status == 'Pending' ? 'selected' : '' }} >Pending</option>
                  <option value="Ordered" {{ $purchasedata->status == 'Ordered' ? 'selected' : '' }} >Ordered</option>
               </select>
               @error('status')
                  <span class="text-danger">{{ $message }}</span>
               @enderror
      ...
      <div class="col-md-12 mt-2">
         ...
         <textarea class="form-control" name="note" rows="3" placeholder="Enter Notes">{{ $purchasedata->note }}</textarea>
      </div>
   </div>
</div>
</div>
